...
class Fruit():
    ...
    @property
    def was_collected(self): return self.__was_collected and self.__frame_index >= len(self.__animations[COLLECTED]) - 1
    ...
    def __create_animations(self, data):
        for animation in data['animations']:
            self.__animations[animation] = self.create_animation(data['animations'][animation])

    def create_animation(self, data):
        return Auxiliar.getSurfaceFromSpriteSheet(PATH_SOURCES+data['path'], data['cols'], data['rows'])
