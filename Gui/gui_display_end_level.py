...
class DisplayEndLevel(Form):
    def __init__(self, name, master_surface, pos, size, image_bg, color_bg, color_border, active, scale):
        ...
        self.__image_header = py.image.load(PATH_HEADER_WIN)
        self.__image_header = py.transform.rotozoom(self.__image_header, 0, scale).convert_alpha()
        self.__rect_image_header = self.__image_header.get_rect(midtop=(W_MENU/2, 60))
